...
WHERE
  ...
  /* Проверка условия в предложении SELECT */  
    SELECT
        ...
        end as 'Наличие документов', `students`.`name`'ФИО студента'
    FROM
		students
...
SELECT
	...
	end as 'Устаревшие шифры направлений', `groups`.`napr`'Направление подготовки'
FROM
	`groups`
	where 1;
/* Упрощенная форма работы с совпадающми значениями - заменяет второй параметр функции nullif() на значение NULL, если он совпадает с содержимым ячейки */    
SELECT
...
/* Самостоятельный запрос на стортировку значений */
SELECT
	*
FROM
	discipl
ORDER BY pnamedisc DESC;
